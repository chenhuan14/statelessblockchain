...
from main import setup, batch_add, batch_prove_membership, batch_prove_membership_with_NIPoE, \
    ...
    create_all_membership_witnesses, prove_membership, delete, acc_del_mem_witness_update,verify_membership, \
    prove_non_membership, verify_non_membership, add, acc_add_non_membership_witness_update, \
    acc_batch_add_prime_non_membership_witness_update,\
    calculate_primes_product,pal_prove_non_membership
...
acc_non_membership_update_result = []
acc_non_membership_update_result_time = []
...
def test_mem_witness_up_on_delete(total_utxo_set_size_for_accumulator, delete_element_index):
    ...
    for i in range(len(elements_for_accumulator)):
        if i != delete_element_index:
            other_element_proofs[elements_for_accumulator[i]] = prove_membership(A0, S, elements_for_accumulator[i], n)
    tok = time.time()
    print("<-- Done.", tok-tik)
    ...
    for x, proof in other_element_proofs.items():
        update_proofs[x] = acc_del_mem_witness_update(x, proof, delete_element, A_del, n)
    tok = time.time()
    print("<-- Done.", tok-tik)
...
def test_non_membership_witness_update_on_add(total_utxo_set_size_for_accumulator):
    print("total_utxo_set_size_for_accumulator =", total_utxo_set_size_for_accumulator)
    ...
    if test_element in S.keys():
        print("test_element is included in accumulator, change it to other value!")
    ...
    print("--> add random element and update non-membership witness for", test_element)
    element_add = random.randint(1, pow(2, 256))
    A_add = add(A, S, element_add, n)
    d, b = acc_add_non_membership_witness_update(A, S, test_element, d, b, element_add, n)
    print("<-- Done.")
...
def test_non_membership_witness_update_on_batch_add(total_utxo_set_size_for_accumulator, add_element_size):
    print("total_utxo_set_size_for_accumulator =", total_utxo_set_size_for_accumulator)
    ...
    d, b = acc_batch_add_prime_non_membership_witness_update(A, S, test_element, d, b, add_elements_product, n, order)
    tok = time.time()
    acc_non_membership_update_result_time.append(tok-tik)
    print("<-- Done.", tok - tik)

    print("--> prove non-membership after add elements for size:", add_element_size)
    result = verify_non_membership(A0, A_add, d, b, test_element, 0, n)
    print("<-- Done.")
    acc_non_membership_update_result.append(result)
...
# test_mem_witness_up_on_delete(10, 2)

for i in range(5):
    test_non_membership_witness_update_on_batch_add(10, (i + 1) * 600)
    print(acc_non_membership_update_result)
    print(acc_non_membership_update_result_time)



# for i in range(10):
#     test_create_all_membership_witnesses(pow(2,i+1))
#     print(acc_create_all_membership_witness)
